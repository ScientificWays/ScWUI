...
#include "ScWPrimaryGameLayout.generated.h"
...
class UScWPrimaryGameLayout : public UCommonUserWidget
{
	GENERATED_BODY()

public:
	static MODULE_API UScWPrimaryGameLayout* GetPrimaryGameLayoutForPrimaryPlayer(const UObject* InWCO);
	static MODULE_API UScWPrimaryGameLayout* GetPrimaryGameLayout(APlayerController* InPlayerController);
	static MODULE_API UScWPrimaryGameLayout* GetPrimaryGameLayout(ULocalPlayer* InLocalPlayer);

public:
	MODULE_API UScWPrimaryGameLayout(const FObjectInitializer& ObjectInitializer);
...
public:
	...
	TSharedPtr<FStreamableHandle> PushWidgetToLayerStackAsync(FGameplayTag LayerName, bool bSuspendInputUntilComplete, TSoftClassPtr<UCommonActivatableWidget> ActivatableWidgetClass, TFunction<void(EAsyncWidgetLayerState, ActivatableWidgetT*)> StateFunc)
	{
		...
		const FName SuspendInputToken = bSuspendInputUntilComplete ? UScWUIFunctionLibrary::SuspendInputForPlayer(GetOwningPlayer(), NAME_PushingWidgetToLayer) : NAME_None;

		FStreamableManager& StreamableManager = UAssetManager::Get().GetStreamableManager();
		TSharedPtr<FStreamableHandle> StreamingHandle = StreamableManager.RequestAsyncLoad(ActivatableWidgetClass.ToSoftObjectPath(), FStreamableDelegate::CreateWeakLambda(this,
			[this, LayerName, ActivatableWidgetClass, StateFunc, SuspendInputToken]()
			{
				UScWUIFunctionLibrary::ResumeInputForPlayer(GetOwningPlayer(), SuspendInputToken);
	...
	MODULE_API void FindAndRemoveWidgetFromLayer(UCommonActivatableWidget* ActivatableWidget);
...
protected:
	...
	MODULE_API void OnWidgetStackTransitioning(UCommonActivatableWidgetContainerBase* Widget, bool bIsTransitioning);
